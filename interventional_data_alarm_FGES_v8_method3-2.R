...
# version 8 to integrate with bayesian prob with depenc or indepen with uniform and random 5 each of datapoint
...
for(iteration in 2:n_iter)
{
  ...
  for ( data_j in 1:nrow(E)) 
    {
        ...
        for(i_bic in 0:as.integer(nodes$size()-1))
        {
          ...
          local_BIC[i_bic+1,data_j+1] <-nodes$get(as.integer(i_bic))$getAttribute('BIC')
          local_BIC[n+1,data_j+1]<- nodesx$getAttribute('BIC')
  ...
  # 
  for ( data_index in 1:nrow(E))
  {

    for ( var_index in 1: length(Data[[data_index]][["e"]]))
    {

      # if(ratio_BIC[var_index,data_index+1] <= thred3)
      # {
      #   ratio_BIC[var_index,data_index+1] <- 0
      # }
      if(ratio_BIC[var_index,data_index+1] >1 )
      {
        ratio_BIC[var_index,data_index+1] <- 1
      }
...
#soft contrain v-structue integrated to prior
    ...
    for(index_sepset_i in 1:n)
    {
      for(index_sepset_j in 1:n)
      {
        ...
        if (size !=0)
        {
          ...
          if (length(a) ==1 &  rule0_graph[["sepsetratio"]][[index_sepset_i]][[index_sepset_j]] <thred2)
          ...
          else if  (length(a) >1 & rule0_graph[["sepsetratio"]][[index_sepset_i]][[index_sepset_j]] <thred2)
    ...
    #############BOX3
    for ( data_index in 1:(nrow(E)-1) )
    {
      
      for ( var_index in 1: length(Data[[data_index]][["e"]]))
      {
        
        if( Data[[data_index]][["e"]][var_index] == 1 )
          
        { 
          
          
           #possDec <- possDe(m = t(essential_amat), x = var_index, possible = TRUE, ds = FALSE, type = "cpdag")
           # for (possDec_index in possDec )
           # {
           #   print(colnames(essential_amat)[possDec_index])
           # }
          
          print(paste0(data_index ," have ",colnames(amat)[var_index]))
          for (row_index in 1:length(Data[[data_index]][["e"]]) )
          {
          
            
            if (row_index !=var_index)
            {
              #posterior_obs[var_index,row_index] <- max(posterior_obs[var_index,row_index],ratio_BIC[var_index,data_index+1])

              
              if(as.double(ratio_BIC[row_index,data_index+1]) <thred3 & (amat_undirect[var_index,row_index]==3 | amat_undirect[row_index,var_index]==3) )
              {
                #print(paste0("index=",row_index,data_index+1,"BIC=", ratio_BIC[row_index,data_index+1]))
                print(paste0(colnames(amat)[row_index]," reverse ",  colnames(amat)[var_index]))
                #amat[row_index,var_index] <- max(amat[row_index,var_index]-as.double(ratio_BIC[row_index,data_index+1]),0)
                amat[row_index,var_index] <-1
              }
              else if (as.double(ratio_BIC[row_index,data_index+1]) >thred3 & essential_amat[var_index,row_index]==1)
              {
               
                amat[var_index,row_index] <- min(amat[var_index,row_index]+as.double(ratio_BIC[row_index,data_index+1]),1)
                print(paste0(" enhance ",colnames(amat)[var_index]," -> ",  colnames(amat)[row_index],' by ', as.double(ratio_BIC[row_index,data_index+1])))
...
# catch Bdeu score for constraints
   ...
   rownames(posterior_obs) <- colnames(dataset)
   colnames(posterior_obs) <- colnames(dataset)
...
#############################################
  ...
  # 
    ...
    # 
    # source("graphutils.R")
    #  #graph_dot <- tetradrunner.tetradGraphToDot(Data[[35]][["pag"]][["graph"]])
    # #
