...
set <-'trainingData_ALARM_L10_10k.csv'
...
dataset$HISTORY <- as.factor(dataset$HISTORY)
dataset$LVFAILURE <- as.factor(dataset$LVFAILURE)
...
#construct true MAG
true_mag = empty.graph(c("ANAPHYLAXIS","ARTCO2","BP","CATECHOL","CO","CVP","DISCONNECT","ERRCAUTER","ERRLOWOUTPUT","EXPCO2","FIO2","HISTORY","HR","HRBP","HREKG","HRSAT","HYPOVOLEMIA","INSUFFANESTH","INTUBATION","KINKEDTUBE","LVEDVOLUME","LVFAILURE","MINVOL","PAP","PCWP","PRESS","PULMEMBOLUS","PVSAT","SAO2","SHUNT","STROKEVOLUME","VENTMACH","VENTTUBE"))


arc.set = matrix(c("ARTCO2","CATECHOL","ARTCO2","EXPCO2","CATECHOL","HR","CO","BP","DISCONNECT","VENTTUBE","ERRCAUTER","HREKG","ERRCAUTER","HRSAT","ERRLOWOUTPUT","HRBP","FIO2","PVSAT","HR","CO","HR","HRBP","HR","HREKG","HR","HRSAT","HYPOVOLEMIA","LVEDVOLUME","HYPOVOLEMIA","STROKEVOLUME","INSUFFANESTH","CATECHOL","INTUBATION","MINVOL","INTUBATION","PRESS","INTUBATION","SHUNT","KINKEDTUBE","PRESS","LVEDVOLUME","CVP","LVEDVOLUME","PCWP","LVFAILURE","HISTORY","LVFAILURE","LVEDVOLUME","LVFAILURE","STROKEVOLUME","PULMEMBOLUS","PAP","PULMEMBOLUS","SHUNT","PVSAT","SAO2","SAO2","CATECHOL","SHUNT","SAO2","STROKEVOLUME","CO","VENTMACH","VENTTUBE","VENTTUBE","PRESS","EXPCO2","MINVOL","EXPCO2","PVSAT","MINVOL","ARTCO2","MINVOL","PVSAT","ARTCO2","PVSAT","KINKEDTUBE","EXPCO2","KINKEDTUBE","MINVOL","INTUBATION","EXPCO2","VENTTUBE","EXPCO2","VENTTUBE","MINVOL","INTUBATION","ARTCO2","INTUBATION","PVSAT","KINKEDTUBE","ARTCO2","KINKEDTUBE","PVSAT","VENTTUBE","ARTCO2","VENTTUBE","PVSAT","CATECHOL","BP","ANAPHYLAXIS","BP","ANAPHYLAXIS","CATECHOL","ARTCO2","BP","INSUFFANESTH","BP","SAO2","BP","MINVOL","EXPCO2","PVSAT","EXPCO2","ARTCO2","MINVOL","PVSAT","MINVOL","PVSAT","ARTCO2"),ncol = 2, byrow = TRUE,dimnames = list(NULL, c("from", "to")))
...
true_mag_amat[10,23] <-1
true_mag_amat[23,10] <-1

true_mag_amat[10,28] <-1
true_mag_amat[28,10] <-1

true_mag_amat[23,2] <-1
true_mag_amat[2,23] <-1

true_mag_amat[23,28] <-1
true_mag_amat[28,23] <-1

true_mag_amat[2,28] <-1
true_mag_amat[28,2] <-1
...
for(iteration in 2:n_iter)
{
  ...
  for ( i in 1:nrow(E)) {
    ...
    if (length(J) !=0)
    {
      ...
      interventional_dataset$VENTLUNG <- NULL
      interventional_dataset$MINVOLSET <- NULL
      interventional_dataset$VENTALV <- NULL
      interventional_dataset$TPR <- NULL
  ...
  # plain model averaging concept from frequentist approach
    ...
    for ( i in 1:nrow(E)) {
      ...
      { 
        amat_undirect_temp <- matrix(0, n, n)
        ...
        amat_undirect_temp [ amat_temp ==t(amat_temp) &  amat_temp ==3 ] <- 1
        ...
        # else{
        amat <- amat+amat_temp
        amat_undirect <- amat_undirect + amat_undirect_temp
        amat_undirect_temp <- matrix(0, n, n)
      }
   
    }  
    amat <- (amat/(nrow(E)))
    amat_undirect  <- (amat_undirect/(nrow(E)))
...
# catch Bdeu score for constraints
   ...
   indep_graphs = empty.graph(c("ANAPHYLAXIS","ARTCO2","BP","CATECHOL","CO","CVP","DISCONNECT","ERRCAUTER","ERRLOWOUTPUT","EXPCO2","FIO2","HISTORY","HR","HRBP","HREKG","HRSAT","HYPOVOLEMIA","INSUFFANESTH","INTUBATION","KINKEDTUBE","LVEDVOLUME","LVFAILURE","MINVOL","PAP","PCWP","PRESS","PULMEMBOLUS","PVSAT","SAO2","SHUNT","STROKEVOLUME","VENTMACH","VENTTUBE"))
...
#############################################
  ...
  true_mag_amat <- amat(true_mag)
  undirect_gt <-((t(true_mag_amat)-true_mag_amat ==0)) & (true_mag_amat==1)
  undirect_learn <- ((t(amat)-amat ==0)) & (amat==1)
  undirect_gts <-0.5*length(undirect_gt[undirect_gt==TRUE])
  undirect_learns <-0.5*length(undirect_learn[undirect_learn==TRUE])
    
  true_mag_amat<- true_mag_amat[order(rownames(true_mag_amat)),order(colnames(true_mag_amat))]
  #amat_round <- round(amat)

  k <-precisionrecall(posterior_obs, true_mag_amat)
