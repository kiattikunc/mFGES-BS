...
# version 6 to integrate conservative rule with majority rule for v-structures and bidirected edges
...
for(iteration in n_iter)
{
  ...
  # plain model averaging concept from frequentist approach
   ...
   ### BOX2: use the skeleton from BOX1 and perform the statistic test to identify v-structure using conservative rule with majority rule
    ...
    i <- n_iter+1 # use observational dataset to identify skeleton and v-structure
...
#hard contrain v-structue integrated to Box1
   ...
   for(index_v in 1:ncol(vstrucr_true))
     ...
     if (is.na(vstrucr_true[4,index_v]) ==FALSE)
     {
       if (vstrucr_true[4,index_v]==TRUE &&  amat[vstrucr_true[1,index_v],vstrucr_true[2,index_v]]==0)
           {
        
             #amat[vstrucr_true[1,index_v],vstrucr_true[2,index_v]] <-1
       }
     
       if (vstrucr_true[4,index_v]==TRUE &&  amat[vstrucr_true[3,index_v],vstrucr_true[2,index_v]]==0)
     {
             #amat[vstrucr_true[3,index_v],vstrucr_true[2,index_v]] <-1
     }
...
# catch Bdeu score for constraints
   ...
   res = set.arc(indep_graphs, "STROKEVOLUME", "LVEDVOLUME")
   
   depend_score <- score(res,Data[[i]]$data, type = "bde")
   
   ## let's see score equivalence in action!
   plot(res2)
   res2 = set.arc(indep_graphs, "LVEDVOLUME", "STROKEVOLUME")
   score(res2, Data[[i]]$data, type = "bde")
   res3 <- drop.arc(res2, "LVEDVOLUME", "STROKEVOLUME")
   indepence_score <- score(res3, Data[[1]]$data, type = "bde")
   prior_left <- 1/3
   prior_right <- 1/3
   prior_indepence <- 1/3
   depence_posterior <- ((prior_left + prior_right)*depend_score)/( ((prior_left + prior_right)*depend_score)+prior_indepence* indepence_score)
