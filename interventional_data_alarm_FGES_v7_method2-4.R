...
for(iteration in 2:n_iter)
{
  ...
  for ( data_index in 1:nrow(E)) 
  {
    ...
    {
      ratio_BIC[var_index,data_index+1] <- abs(as.double(ratio_BIC[var_index,data_index+1])*(1-Data[[data_index]][["e"]][var_index]))
      
      if(ratio_BIC[var_index,data_index+1] <= thred2)
      {
...
#essential_amat <- TetradGetAdjmat(Data[[iteration+1]]$pag)
  ...
  essential_amat [ amat_undirect ==3] <- 1
    ...
    for ( var_index in 1: length(Data[[data_index]][["e"]]))
    {
      ...
      { 
        ...
        for (row_index in 1:length(Data[[data_index]][["e"]]) )
        {
          ...
          if(ratio_BIC[var_index,data_index+1] ==0 & amat_undirect[var_index,row_index]==3 )
          {
            
            #posterior_obs[row_index,var_index] <-1
          }
  ...
  iterFGES$nUndirectEdges[[iteration]] <- k[["nUndirectEdges"]]
  ...
  set6 <- paste0(n_iter,"-nUndirectedges.csv")
  ...
  write.csv(iterFGES$nUndirectEdges,paste0(path,set6))
